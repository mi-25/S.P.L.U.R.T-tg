	...
	if(!pinless)
		if(pin)
			...
			if(pin.pin_removable)
				. += span_info("[pin]看起来可以用一些<b>工具</b>移除。")
			else
				. += span_info("[pin]看起来被牢牢锁住了，似乎不可能移除。")
		else
			. += "它没有安装<b>击发针</b>，无法射击。"
...
/obj/item/gun/proc/shoot_live_shot(mob/living/user, pointblank = FALSE, atom/pbtarget = null, message = TRUE)
	...
	if(tk_firing(user))
		visible_message(
			span_danger("[src]自己开火了[pointblank ? "，近距离射击[pbtarget]！" : "！"]"),
			blind_message = span_hear("你听到了枪声！"),
			vision_distance = COMBAT_MESSAGE_RANGE
		)
	else if(pointblank)
		if(user == pbtarget)
			user.visible_message(
				span_danger("[user]用[src]近距离射击了[user.p_them()]自己！"),
				span_userdanger("你用[src]近距离射击了自己！"),
				span_hear("你听到了枪声！"),
				vision_distance = COMBAT_MESSAGE_RANGE,
			...
			user.visible_message(
				span_danger("[user]用[src]近距离射击了[pbtarget]！"),
				span_danger("你用[src]近距离射击了[pbtarget]！"),
				span_hear("你听到了枪声！"),
				vision_distance = COMBAT_MESSAGE_RANGE,
				ignored_mobs = pbtarget,
				visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
			)
			to_chat(pbtarget, span_userdanger("[user]用[src]近距离射击了你！"))
		...
		user.visible_message(
			span_danger("[user]用[src]开火了！"),
			span_danger("你用[src]开火了！"),
			span_hear("你听到了枪声！"),
			vision_distance = COMBAT_MESSAGE_RANGE,
			ignored_mobs = user,
			visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
...
/obj/item/gun/attack_self_secondary(mob/user, modifiers)
	...
	if(HAS_TRAIT(user, TRAIT_CLUMSY) && prob(40))
		...
		user.visible_message(
			span_danger("在试图转动[src]时，[user]不小心扣动了扳机！"),
			span_userdanger("在试图转动[src]时，你不小心扣动了扳机！"),
		)
		process_fire(user, user, FALSE, user.get_random_valid_zone(even_weights = TRUE))
		user.dropItemToGround(src, TRUE)
	else
		user.visible_message(
			span_notice("[user]用扳机转动[src]绕着[user.p_their()]手指旋转。真是帅气。"),
			span_notice("你用扳机转动[src]绕着手指旋转。真是帅气。"),
		)
...
/obj/item/gun/interact_with_atom_secondary(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if (gunpoint_component)
		balloon_alert(user, "已经在劫持[gunpoint_component.target == interacting_with ? "他们" : "某人"]了！")
		return ITEM_INTERACT_BLOCKING
	if (user == interacting_with)
		balloon_alert(user, "不能劫持自己！")
		return ITEM_INTERACT_BLOCKING
...
/obj/item/gun/proc/handle_pins(mob/living/user)
	...
	if(pin)
		...
		to_chat(user, span_warning("[src]的扳机被锁住了。这把武器没有安装击发针！"))
		balloon_alert(user, "扳机锁定，需要击发针！")
	return FALSE
...
/obj/item/gun/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(pin?.pin_removable && user.is_holding(src))
		...
		span_notice("你试图从[src]中移除[pin]。（需要[DisplayTimeText(FIRING_PIN_REMOVAL_DELAY)]。）"), null, 3)
		if(I.use_tool(src, user, FIRING_PIN_REMOVAL_DELAY, volume = 50))
			if(!pin) //check to see if the pin is still there, or we can spam messages by clicking multiple times during the tool delay
				return
			user.visible_message(span_notice("[user]用撬棍从[src]中撬出了[pin]，在过程中摧毁了击发针。"),
								span_warning("你用[I]撬出了[pin]，在过程中摧毁了击发针。"), null, 3)
			QDEL_NULL(pin)
			return ITEM_INTERACT_SUCCESS
...
/obj/item/gun/welder_act(mob/living/user, obj/item/I)
	...
	if(pin?.pin_removable && user.is_holding(src))
		...
		if(I.use_tool(src, user, FIRING_PIN_REMOVAL_DELAY, 5, volume = 50))
			if(!pin) //check to see if the pin is still there, or we can spam messages by clicking multiple times during the tool delay
				return
			user.visible_message(span_notice("[user]用焊枪从[src]中切出了[pin]，在过程中熔化了部分击发针。"),
								span_warning("你用[I]从[src]中切出了[pin]，在过程中熔化了部分击发针。"), null, 3)
			QDEL_NULL(pin)
			return TRUE
...
/obj/item/gun/wirecutter_act(mob/living/user, obj/item/I)
	...
	if(pin?.pin_removable && user.is_holding(src))
		...
		if(I.use_tool(src, user, FIRING_PIN_REMOVAL_DELAY, volume = 50))
			if(!pin) //check to see if the pin is still there, or we can spam messages by clicking multiple times during the tool delay
				return
			user.visible_message(span_notice("[user]用钳子从[src]中扯出了[pin]，在过程中损坏了击发针。"),
								span_warning("你用[I]从[src]中扯出了[pin]，在过程中损坏了击发针。"), null, 3)
			QDEL_NULL(pin)
			return TRUE
...
/obj/item/gun/proc/handle_suicide(mob/living/carbon/human/user, mob/living/carbon/human/target, params, bypass_timer)
	...
	if(user == target)
		target.visible_message(span_warning("[user]把[src]塞进[user.p_their()]嘴里，准备扣动扳机..."), \
			span_userdanger("你把[src]塞进嘴里，准备扣动扳机..."))
	else
		target.visible_message(span_warning("[user]用[src]指着[target]的头，准备扣动扳机..."), \
			span_userdanger("[user]用[src]指着你的头，准备扣动扳机..."))
	...
	if(!bypass_timer && (!do_after(user, 12 SECONDS, target) || user.zone_selected != BODY_ZONE_PRECISE_MOUTH))
		if(user)
			if(user == target)
				user.visible_message(span_notice("[user]决定不开枪了。"))
			else if(target?.Adjacent(user))
				target.visible_message(span_notice("[user]决定放过[target]"), span_notice("[user]决定饶你一命！"))
		fire_cd = FALSE
		return

	fire_cd = FALSE

	target.visible_message(span_warning("[user]扣动了扳机！"), span_userdanger("[(user == target) ? "你扣动" : "[user]扣动"]了扳机！"))
